<x-layout>
    ...
    <div class="max-w-4xl mx-auto p-6 bg-white rounded-2xl shadow-lg">
        ...
        <a href="{{ route('witness.create') }}" class="mb-4 inline-block bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700">
            {{ __('Add New Witness') }}
        </a>
            ...
            <tbody>
                @foreach($witnesses as $witness)
                    <tr class="border-t">
                        ...
                        <td class="p-2">
                            <a href="{{ route('witness.edit', $witness->id) }}" class="text-blue-600 hover:underline">{{ __('Edit') }}</a>
                            <form action="{{ route('witness.destroy', $witness->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
